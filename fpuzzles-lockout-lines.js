...
(function() {
    ...
    const doShim = function() {
        ...
        const drawLine = function(line, color, colorDark, lineWidth) {
            ...
            ctx.fill();
            ctx.beginPath();
            ctx.translate(line[0].x + cellSL / 2, line[0].y + cellSL * (0.5 - Math.sqrt(2)* 0.55/2));
            ctx.rotate(45 * Math.PI / 180);
            ctx.translate(-line[0].x- cellSL / 2, -line[0].y - cellSL * (0.5 - Math.sqrt(2)* 0.55/2));
            ctx.lineWidth = cellSL * 0.1 * 0.5;
            ctx.strokeStyle = boolSettings['Dark Mode'] ? colorDark : color;
            ctx.fillStyle = boolSettings['Dark Mode'] ? '#000000' : '#FFFFFF';
            ctx.fillRect(line[0].x+ cellSL / 2 , line[0].y + cellSL * (0.5 - Math.sqrt(2)* 0.55/2) , 0.55 * cellSL, 0.55 * cellSL);
            ctx.strokeRect(line[0].x+ cellSL / 2 , line[0].y + cellSL * (0.5 - Math.sqrt(2)* 0.55/2) , 0.55 * cellSL, 0.55 * cellSL);
            ctx.setTransform(1, 0, 0, 1, 0, 0);
            ctx.beginPath();
            ctx.translate(line[line.length - 1].x + cellSL / 2, line[line.length - 1].y + cellSL * (0.5 - Math.sqrt(2)* 0.55/2));
            ctx.rotate(45 * Math.PI / 180);
            ctx.translate(-line[line.length - 1].x- cellSL / 2, -line[line.length - 1].y - cellSL * (0.5 - Math.sqrt(2)* 0.55/2));
            ctx.lineWidth = cellSL * 0.1 * 0.5;
            ctx.strokeStyle = boolSettings['Dark Mode'] ? colorDark : color;
            ctx.fillStyle = boolSettings['Dark Mode'] ? '#000000' : '#FFFFFF';
            ctx.fillRect(line[line.length - 1].x+ cellSL / 2 , line[line.length - 1].y + cellSL * (0.5 - Math.sqrt(2)* 0.55/2) , 0.55 * cellSL, 0.55 * cellSL);
            ctx.strokeRect(line[line.length - 1].x+ cellSL / 2 , line[line.length - 1].y + cellSL * (0.5 - Math.sqrt(2)* 0.55/2) , 0.55 * cellSL, 0.55 * cellSL);
            ctx.setTransform(1, 0, 0, 1, 0, 0);
        }
